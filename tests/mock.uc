	...
	let mocks = {
					...
					call: (object, method, args) => {
						...
						for (let i = length(signature); i > 0; i--) {
							...
							if (mock != mock) {
								...
								printf("ctx.call() object <%s> method <%s> args <%s>\n", object, method, format_json(args));
		...
		uci: {
			cursor: () => ({
				...
				load: function(file) {
					let path = sprintf("./tests/mocks/uci/%s.json", file),
					    mock = read_json_file(path);
		...
		fs: {
			readlink: function(path) {
				printf("fs.readlink() path <%s>\n", path);

				return path + "-link";
			},

			stat: function(path) {
				printf("fs.stat() path <%s>\n", path);
		...
		system: function(argv, timeout) {
			printf("system() command <%J> timeout <%s>\n", argv, timeout);
